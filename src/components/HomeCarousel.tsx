...
export function HomeCarousel() {
  return (
    <Carousel
      ...
      className='relative my-0 mt-4 mt:mt-12 px-4 md:px-0 '
    >
      <CarouselContent>
        {slides.map((slide) => (
          <CarouselItem
            key={slide.id}
            className='md:basis-[60.666%] gap-x-6 md:ml-7 lg:ml-12 h-full'
          >
            <Card
              style={{
            ...
            >
              <CardContent className='text-white h-full flex flex-col justify-between '>
                ...
                <div className='flex items-end justify-between'>
                  <div>
                    <p className=' hidden md:block text-[clamp(0.9rem,2vw,2rem)]'>
                      {slide.footerTittle}
                    </p>

                    <p className='max-w-[60%] hidden md:block text-[clamp(0.45rem,1vw,1.125rem)] leading-snug'>
                      {slide.description}
                    </p>
                  </div>
